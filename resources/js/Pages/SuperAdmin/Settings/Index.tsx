...
import { Globe, CreditCard, Mail, MapPin, Wrench, HardDrive, Bell, ShieldCheck, Save, Upload, CheckCircle, Eye, EyeOff, AlertTriangle, Trash2, Plus, Images } from 'lucide-react';

interface RegisterBgItem {
    path: string;
    url: string;
}
...
const TIMEZONES = [
    'Africa/Nairobi', 'UTC', 'Asia/Kolkata', 'Asia/Karachi', 'Asia/Dubai',
    'America/New_York', 'America/Chicago', 'America/Los_Angeles',
    'Europe/London', 'Europe/Berlin', 'Africa/Cairo', 'Australia/Sydney',
];
const CURRENCIES = ['KES','USD','EUR','GBP','BDT','INR','PKR','AED','SAR','EGP','AUD','CAD'];
...
function GeneralTab({ settings, logoUrl, faviconUrl, loginBgUrl, registerBgUrls = [] }: Props) {
    const form = useForm<{
        ...
        register_backgrounds: File[] | null;
    }>({
        platform_name: settings.platform_name ?? 'EduFlow',
        ...
        register_backgrounds: null,
    });

    const [logoPreview, setLogoPreview] = useState<string | null>(logoUrl);
    const [faviconPreview, setFaviconPreview] = useState<string | null>(faviconUrl);
    const [loginBgPreview, setLoginBgPreview] = useState<string | null>(loginBgUrl);
    ...
    const registerBgRef = useRef<HTMLInputElement>(null);
    const [newRegPreviews, setNewRegPreviews] = useState<string[]>([]);
    const { flash } = usePage<any>().props;

    function handleFile(field: 'logo' | 'favicon' | 'login_background', file: File | null) {
        if (!file) return;
        form.setData(field, file);
        const url = URL.createObjectURL(file);
        if (field === 'logo') setLogoPreview(url);
        if (field === 'favicon') setFaviconPreview(url);
        if (field === 'login_background') setLoginBgPreview(url);
    }
        ...
        <form onSubmit={e => { e.preventDefault(); form.post('/super-admin/settings/general', { forceFormData: true,  forceFormData: true }); }}>
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Configure the platform name, logo, favicon, and login artwork.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    {flash?.success && <SuccessBanner message={flash.success} />}

                    {/* Logo, Favicon & Login Background Upload Cards */}
                    <div className="flex flex-wrap gap-6 items-start">
                        {/* Platform Logo */}
                        <div>
                            <Label className="block mb-2 text-xs font-semibold">Platform Logo</Label>
                            <div className="flex items-center gap-3">
                                <div className="w-36 h-24 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-slate-800 p-2">
                                    {logoPreview ? (
                                ...
                                <div>
                                    <input ref={logoRef} type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" className="hidden" onChange={e => handleFile('logo', e.target.files?.[0] ?? null)} />
                                    <Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => logoRef.current?.click()}>
                                        <Upload className="w-3.5 h-3.5" /> Upload
                                    </Button>
                                    <p className="text-[11px] text-slate-400 mt-1">PNG/SVG, max 2MB</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Favicon */}
                        <div>
                            <Label className="block mb-2 text-xs font-semibold">Favicon</Label>
                            <div className="flex items-center gap-3">
                                <div className="w-24 h-24 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-slate-800 p-2">
                                    {faviconPreview ? (
                                ...
                                <div>
                                    <input ref={faviconRef} type="file" accept="image/x-icon,image/png,image/svg+xml,image/jpeg" className="hidden" onChange={e => handleFile('favicon', e.target.files?.[0] ?? null)} />
                                    <Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => faviconRef.current?.click()}>
                                        <Upload className="w-3.5 h-3.5" /> Upload
                                    </Button>
                                    <p className="text-[11px] text-slate-400 mt-1">Max 512KB</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <Label className="block mb-2 text-xs font-semibold">Login Background</Label>
                            <div className="flex items-center gap-3">
                                <div className="w-36 h-24 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden bg-slate-900">
                                    {loginBgPreview ? (
                                        <img src={loginBgPreview} alt="Login Background" className="w-full h-full object-cover" />
                                    ) : (
                                        <span className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider text-center px-1">KICC Skyline</span>
                                    )}
                                </div>
                                <div>
                                    <input ref={loginBgRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={e => handleFile('login_background', e.target.files?.[0] ?? null)} />
                                    <Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => loginBgRef.current?.click()}>
                                        <Upload className="w-3.5 h-3.5" /> Upload
                                    </Button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-4">
                        ...
                        <div>
                            ...
                            <Input value={form.data.footer_copyright} onChange={e => form.setData('footer_copyright', e.target.value)} />
                                    ...
                                    {/* Registration Multi-Image Autoscroller Backgrounds */}
                    <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
                        <div className="flex items-center justify-between mb-2">
                            <div>
                                <Label className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                    <Images className="w-4 h-4 text-emerald-600" />
                                    Registration Page Slideshow Backdrops (/register)
                                </Label>
                                <p className="text-xs text-slate-500 mt-0.5">
                                    Upload multiple campus photos. The registration onboarding wizard will automatically cross-fade between them.
                                </p>
                            </div>
                            <Button
                                type="button"
                                variant="outline"
                                size="sm"
                                className="gap-2 border-emerald-300 text-emerald-700 hover:bg-emerald-50 dark:hover:bg-slate-800"
                                onClick={() => registerBgRef.current?.click()}
                            >
                                <Plus className="w-3.5 h-3.5" /> Add Slides
                            </Button>
                            <input
                                ref={registerBgRef}
                                type="file"
                                ...
                                onChange={(e) => {
                                    ...
                                    if (files.length > 0) {
                                        form.setData('register_backgrounds', files);
                                        const urls = files.map((f) => URL.createObjectURL(f));
                                        setNewRegPreviews(urls);
                                    }
                                }}
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3 mt-4">
                            {registerBgUrls && registerBgUrls.length > 0 ? (
                                registerBgUrls.map((item, idx) => (
                                    <div key={idx} className="relative group rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 aspect-video shadow-xs">
                                        ...
                                        <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                            <button
                                                ...
                                                onClick={() => {
                                                    if (confirm('Delete this registration background slide?')) {
                                                        router.post('/super-admin/settings/register-backgrounds/delete', { path: item.path });
                                                    }
                                                }}
                                            >
                                ...
                                <div className="col-span-full py-6 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50/50 dark:bg-slate-800/30">
                                    <p className="text-xs text-slate-400">No registration slideshow backdrops uploaded yet. (Falls back to default login artwork or dark institutional mesh).</p>
                                </div>
                            )}

                            {/* Pending Upload Previews */}
                            {newRegPreviews.map((previewUrl, idx) => (
                                <div key={`new-${idx}`} className="relative rounded-xl overflow-hidden border-2 border-emerald-500 aspect-video shadow-sm">
                                    <img src={previewUrl} alt="New Slide" className="w-full h-full object-cover opacity-80" />
                                    <span className="absolute top-1 right-1 px-1.5 py-0.5 bg-emerald-600 text-[9px] font-bold text-white rounded shadow-xs">
                                        Ready to Save
                                    </span>
                                </div>
                            ))}
                ...
                <div className="px-6 pb-6 flex justify-end">
                    <Button type="submit" disabled={form.processing} className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">
...
function PaymentTab({ settings }: { settings: Record<string, string> }) {
    ...
    const form = useForm({
        ...
        paystack_webhook:   settings.paystack_webhook   ?? '',
        ...
        <form onSubmit={e => { e.preventDefault(); form.post('/super-admin/settings/payment', { forceFormData: true,  forceFormData: true, preserveScroll: true }); }}>
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Configure Paystack API keys for card tokenization and M-Pesa billing.</CardDescription>
                </CardHeader>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <Input autoComplete="new-password" value={form.data.platform_mail_username} onChange={e => form.setData('platform_mail_username', e.target.value)} />
                        </div>
                        <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <Input type="email" value={form.data.platform_mail_from_address}
                                ...
                                placeholder="noreply@eduflow.example.com" />
                        </div>
                        <div>
            ...
            <div className="space-y-6">
                ...
                <div className="flex gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                        {activeTab === 'general' && <GeneralTab settings={settings} logoUrl={logoUrl} faviconUrl={faviconUrl} loginBgUrl={loginBgUrl} registerBgUrls={registerBgUrls} />}
