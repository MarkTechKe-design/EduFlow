...
import { Settings, Building2, Palette, GraduationCap, Bell, Save, Upload, CheckCircle } from 'lucide-react';

interface SchoolData {
    id: number; name: string; email: string | null; phone: string | null;
    address: string | null; city: string | null; state: string | null;
    county?: string | null; sub_county?: string | null; knec_code?: string | null; registration_number?: string | null;
...
const CURRENCIES = ['KES','USD','EUR','GBP','BDT','USD','EUR','GBP','INR','PKR','AED','SAR','EGP','AUD','CAD'];
...
function GeneralTab({ school }: { school: SchoolData }) {
    const form = useForm({
        ...
        county: school.county ?? '', sub_county: school.sub_county ?? '',
        knec_code: school.knec_code ?? '', registration_number: school.registration_number ?? '',
            ...
            <Card>
                ...
                <CardContent className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <Label>County</Label>
                            <Input placeholder="e.g. Nairobi" value={form.data.county} onChange={e => form.setData('county', e.target.value)} />
                            {form.errors.county && <p className="text-xs text-red-500 mt-1">{form.errors.county}</p>}
                        </div>
                        <div>
                            <Label>Sub-County</Label>
                            <Input placeholder="e.g. Westlands" value={form.data.sub_county} onChange={e => form.setData('sub_county', e.target.value)} />
                            {form.errors.sub_county && <p className="text-xs text-red-500 mt-1">{form.errors.sub_county}</p>}
                        </div>
                        <div>
                            <Label>KNEC Code</Label>
                            <Input placeholder="e.g. 20401001" value={form.data.knec_code} onChange={e => form.setData('knec_code', e.target.value)} />
                            {form.errors.knec_code && <p className="text-xs text-red-500 mt-1">{form.errors.knec_code}</p>}
                        </div>
                        <div>
                            <Label>Registration Number / NEMIS</Label>
                            <Input placeholder="e.g. MOE/PRI/2026/001" value={form.data.registration_number} onChange={e => form.setData('registration_number', e.target.value)} />
                            {form.errors.registration_number && <p className="text-xs text-red-500 mt-1">{form.errors.registration_number}</p>}
                        </div>
                        <div>
                            <Label>Country</Label>
