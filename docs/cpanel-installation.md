...
This guide walks you through installing EduFlow on a shared hosting account that uses cPanel. Most popular hosting providers (Namecheap, Hostinger, SiteGround, GreenGeeks, etc.) use cPanel.
...
3. Under **Create New Database**, enter a name (e.g. `youruser_eduflow`) and click **Create Database**.
...
1. Download the EduFlow release ZIP from GitHub.
2. In cPanel, open **File Manager**.
3. Navigate to your home directory (`/home/yourusername/`).
4. Create a new folder called `eduflow` (outside `public_html` — this is important for security).
5. Upload the ZIP file into `eduflow` and extract it.

Your structure should look like:
```
/home/yourusername/
  eduflow/          ← application files live here
    app/
    bootstrap/
    config/
    database/
...
```bash
cd ~
git clone https://github.com/your-org/eduflow.git eduflow
```
...
3. Set the **Document Root** to `/home/yourusername/eduflow/public`.
4. Save.

### Option B — Use Your Main Domain

1. In cPanel, go to **File Manager** and open `public_html`.
2. Move or copy everything from `eduflow/public/` into `public_html/`.
3. Edit `public_html/index.php` — change the two paths at the top to point to your app folder:

```php
require __DIR__.'/../eduflow/vendor/autoload.php';
$app = require_once __DIR__.'/../eduflow/bootstrap/app.php';
```

4. Move the `.htaccess` from `eduflow/public/` into `public_html/` as well.
...
In **File Manager**, navigate to `/home/yourusername/eduflow/`.
...
Then upload the entire `public/build/` folder to `eduflow/public/build/` on the server using File Manager or FTP.
...
```bash
chmod -R 775 ~/eduflow/storage
chmod -R 775 ~/eduflow/bootstrap/cache
```
...
```bash
tail -f ~/eduflow/storage/logs/laravel.log
```
