...
```
/usr/local/bin/php /home/yourusername/eduflow/artisan schedule:run >> /dev/null 2>&1
```

Replace `yourusername` with your actual cPanel username and `eduflow` with the folder name where you installed the app.
...
```
/usr/local/bin/php /home/yourusername/eduflow/artisan queue:work --stop-when-empty >> /dev/null 2>&1
```
