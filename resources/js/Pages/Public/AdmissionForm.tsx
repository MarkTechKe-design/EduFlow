...
interface School { id: number; name: string; logo_url?: string | null; }
interface Branding { name?: string; logo_url?: string | null; }
interface Props { school: School; branding?: Branding; }

export default function AdmissionForm({ school, branding }: Props) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center p-4">
            <div className="w-full max-w-lg">
                ...
                <div className="text-center mb-8">
                    <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-indigo-600 mx-auto mb-4 overflow-hidden">
                        {(school.logo_url || branding?.logo_url) ? <img src={school.logo_url || branding?.logo_url || ''} alt={school.name} className="w-full h-full object-contain bg-white" /> : <GraduationCap className="w-8 h-8 text-white" />}
                    </div>
                ...
                <p className="text-center text-xs text-slate-400 mt-6">
                    Powered by <span className="font-medium text-slate-500">{branding?.name || 'EduFlow'}</span>
                </p>
            </div>
        </div>
    );
