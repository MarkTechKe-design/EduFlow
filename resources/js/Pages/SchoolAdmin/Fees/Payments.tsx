...
export default function FeePayments({ payments, classes, filters, stats }: Props) {
    ...
    const statCards = [
        { label: 'Total Collected', value: `KSh ${stats.total_collected?.toLocaleString() ?? 0}`, color: 'text-green-600', icon: DollarSign },
        { label: 'Outstanding', value: `KSh ${Math.max(0, stats.total_outstanding ?? 0).toLocaleString()}`, color: 'text-red-600', icon: TrendingDown },
                            ...
                            {payments.data.length === 0 ? (
                                ...
                                return (
                                    <TableRow key={p.id}>
                                        ...
                                        <TableCell className="text-right text-sm">KSh {Number(p.amount_due).toLocaleString()}</TableCell>
                                        <TableCell className="text-right text-sm font-semibold text-green-600">KSh {Number(p.amount_paid).toLocaleString()}</TableCell>
                                        <TableCell className={`text-right text-sm font-medium ${Number(balance) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                            {Number(balance) > 0 ? `KSh ${Number(balance).toLocaleString()}` : '—'}
                                        </TableCell>
