...
class SettingsController extends Controller
{
    public function index()
    {
        ...
        $s = SchoolSetting::safeFor($sid);

        return Inertia::render('SchoolAdmin/Settings/Index', [
            'school' => $school->only('id','name','email','phone','address','city','state','country','county','sub_county','knec_code','registration_number','timezone','currency','language','logo'),
    ...
    public function saveGeneral(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'sub_county'          => 'nullable|string|max:60',
    ...
    public function saveBranding(Request $request)
    {
        ...
        $school = School::findOrFail($sid);
        $request->validate([
            'logo' => 'nullable|image|max:2048', 'favicon' => 'nullable|image|max:512',
            'tagline' => 'nullable|string|max:200', 'footer_text' => 'nullable|string|max:500',
            'primary_color' => 'nullable|string|max:20',
        ]);
        if ($request->hasFile('logo')) {
            if ($school->logo) Storage::disk('public')->delete($school->logo);
            $school->update(['logo' => $request->file('logo')->store("schools/{$sid}", 'public')]);
        }
        if ($request->hasFile('favicon')) {
            $old = SchoolSetting::get($sid, 'favicon');
            if ($old) Storage::disk('public')->delete($old);
            SchoolSetting::set($sid, 'favicon', $request->file('favicon')->store("schools/{$sid}", 'public'), 'branding');
        }
        foreach (['tagline', 'footer_text', 'primary_color'] as $key) {
            if ($request->has($key)) SchoolSetting::set($sid, $key, $request->input($key), 'branding');
        }
    ...
    public function saveAcademic(Request $request)
    {
        ...
        $this->authorize('edit', SchoolSetting::class);
        $request->validate([
            'academic_year' => 'nullable|string|max:20', 'year_start' => 'nullable|string|max:10',
    ...
    public function saveNotifications(Request $request)
    {
        ...
        $this->authorize('edit', SchoolSetting::class);
        $toggles = [
            'notify_attendance_email','notify_attendance_sms','notify_fee_due_email','notify_fee_due_sms',
            'notify_exam_email','notify_exam_sms','notify_homework_email','notify_homework_sms',
