...
class LeaveController extends Controller
{
    ...
    public function store(Request $request)
    {
        $sid = $this->getSchoolId();
        $data = $request->validate([
            'staff_id'      => ['required', Rule::exists('staff', 'id')->where(fn ($q) => $q->where('school_id', $sid))],
            'leave_type_id' => ['required', Rule::exists('leave_types', 'id')->where(fn ($q) => $q->where('school_id', $sid))],
