...
use App\Http\Controllers\SchoolAdmin\OnlineClassController;
...
use App\Http\Controllers\SuperAdmin\WebsiteMediaController;
use App\Http\Controllers\SuperAdmin\WebsitePageController;
use App\Http\Controllers\SuperAdmin\WebsitePageSectionController;
use App\Services\LandingRouteResolver;
...
// Dedicated Authentication Entrypoints (Strict Re-Login Enforced)
Route::get('/login', [\App\Http\Controllers\Auth\LoginController::class, 'create'])->name('login');
Route::post('/super-admin/settings/register-backgrounds/delete', [\App\Http\Controllers\SuperAdmin\SettingsController::class, 'deleteRegisterBackground'])->name('super-admin.settings.register-backgrounds.delete');
...
Route::post('/register/validate-coupon', [\App\Http\Controllers\Auth\RegistrationController::class, 'validateCoupon'])->name('register.coupon.validate');

Route::get('/email/verify', fn () => Inertia::render('Auth/VerifyEmail'))->middleware(['auth', 'active'])->name('verification.notice');
Route::get('/email/verify/{id}/{hash}', function (EmailVerificationRequest $request) { $request->fulfill(); return redirect()->route('onboarding.index'); })->middleware(['auth', 'active', 'signed', 'throttle:6,1'])->name('verification.verify');
Route::post('/email/verification-notification', function (Request $request) { $request->user()->sendEmailVerificationNotification(); return back()->with('success', 'A fresh verification link has been sent.'); })->middleware(['auth', 'active', 'throttle:6,1'])->name('verification.send');
...
|--------------------------------------------------------------------------
*/
// Dedicated Public Marketing & Governance Routes
...
// Public Governance & Legal Compliance Suite
...
Route::middleware(['auth', 'active'])->group(function () {
    // Super Admin Marketing & CMS Suite
    Route::prefix('super-admin')->name('super-admin.')->group(function () {
        ...
        Route::put('website/pages/{page}', [\App\Http\Controllers\SuperAdmin\WebsitePageController::class, 'update'])->name('website.pages.update');
        Route::delete('website/pages/{page}', [\App\Http\Controllers\SuperAdmin\WebsitePageController::class, 'destroy'])->name('website.pages.destroy');

        Route::post('website/sections', [\App\Http\Controllers\SuperAdmin\WebsitePageSectionController::class, 'store'])->name('website.sections.store');
        Route::put('website/sections/{section}', [\App\Http\Controllers\SuperAdmin\WebsitePageSectionController::class, 'update'])->name('website.sections.update');
        Route::delete('website/sections/{section}', [\App\Http\Controllers\SuperAdmin\WebsitePageSectionController::class, 'destroy'])->name('website.sections.destroy');

        // Articles & Blog
        Route::get('blogs', [\App\Http\Controllers\SuperAdmin\BlogController::class, 'index'])->name('blogs.index');
        Route::get('blogs/create', [\App\Http\Controllers\SuperAdmin\BlogController::class, 'create'])->name('blogs.create');
        Route::post('blogs', [\App\Http\Controllers\SuperAdmin\BlogController::class, 'store'])->name('blogs.store');
        Route::get('blogs/{blog}/edit', [\App\Http\Controllers\SuperAdmin\BlogController::class, 'edit'])->name('blogs.edit');
        ...
        // Public FAQs
        Route::get('faqs', [\App\Http\Controllers\SuperAdmin\FaqController::class, 'index'])->name('faqs.index');
        Route::post('faqs', [\App\Http\Controllers\SuperAdmin\FaqController::class, 'store'])->name('faqs.store');
        Route::put('faqs/{faq}', [\App\Http\Controllers\SuperAdmin\FaqController::class, 'update'])->name('faqs.update');
        Route::delete('faqs/{faq}', [\App\Http\Controllers\SuperAdmin\FaqController::class, 'destroy'])->name('faqs.destroy');
        ...
        Route::delete('website/media/{media}', [\App\Http\Controllers\SuperAdmin\WebsiteMediaController::class, 'destroy'])->name('website.media.destroy');
    });
    ...
    Route::middleware('verified')->group(function () {
        ...
        Route::post('/billing/package', [BillingController::class, 'changePackage'])->name('billing.package');
        Route::post('/billing/card', [BillingController::class, 'updateCard'])->name('billing.card');
    ...
    Route::get('/dashboard', function () {
        ...
        // Resolve a deterministic existing workspace from the authenticated capabilities.
        return redirect()->route(app(LandingRouteResolver::class)->resolve($user));
    ...
    |--------------------------------------------------------------------------
    */
    Route::middleware(['verified', 'role:school-admin|principal|teacher|accountant|librarian|receptionist|driver|warden|store-manager', 'school-role', 'module'])
        ->prefix('school')
        ->name('school.')
        ->group(function () {
            ...
            // Keep bulk student data work in the school workspace. These routes
            // precede the resource route so import is never model-bound as a student.
            Route::get('students/import', [StudentController::class, 'importView'])->middleware('permission:students.create')->name('students.import');
            Route::get('students/import/template', [StudentController::class, 'downloadTemplate'])->middleware('permission:students.create')->name('students.import.template');
            Route::post('students/import/preview', [StudentController::class, 'previewImport'])->middleware('permission:students.create')->name('students.import.preview');
            Route::post('students/import/process', [StudentController::class, 'processImport'])->middleware('permission:students.create')->name('students.import.process');
        ...
        // Fallback for accidental GET requests on document upload endpoint
        Route::get('students/{student}/documents', function ($student) {
            ...
            Route::get('students/documents/{document}/view', [StudentController::class, 'viewDocument'])->name('students.documents.view');
            ...
            Route::delete('grade-scales/{gradeScale}',       [ExamController::class, 'deleteGradeScale'])->name('grade-scales.destroy');            // Exams & Grade Scales
            ...
            Route::get('exams/{exam}/marks/template', [ExamController::class, 'downloadMarksTemplate'])->name('exams.marks.template');
            Route::post('exams/{exam}/marks/import', [ExamController::class, 'importMarksCsv'])->name('exams.marks.import');
...
// Timetable
            ...
            // HR ?????????????????????????????????????? Leave Management
            Route::get('hr/leave-types',                       [LeaveController::class, 'types'])->name('hr.leave-types.index')->middleware('permission:leave.view');
            ...
            Route::get('hr/leaves',                            [LeaveController::class, 'index'])->name('hr.leaves.index')->middleware('permission:leave.view');
            Route::post('hr/leaves',                           [LeaveController::class, 'store'])->name('hr.leaves.store')->middleware('permission:leave.apply');
            Route::put('hr/leaves/{leaveRequest}/approve',     [LeaveController::class, 'approve'])->name('hr.leaves.approve')->middleware('permission:leave.approve');
            ...
            Route::get('homework',                                    [HomeworkController::class, 'index'])->name('homework.index')->middleware('permission:homework.view');
            Route::post('homework',                                   [HomeworkController::class, 'store'])->name('homework.store')->middleware('permission:homework.create');
            Route::put('homework/{homework}',                         [HomeworkController::class, 'update'])->name('homework.update')->middleware('permission:homework.create');
            Route::delete('homework/{homework}',                      [HomeworkController::class, 'destroy'])->name('homework.destroy')->middleware('permission:homework.create');
            Route::get('homework/{homework}/submissions',             [HomeworkController::class, 'submissions'])->name('homework.submissions')->middleware('permission:homework.view');
            Route::put('homework/submissions/{submission}/review',    [HomeworkController::class, 'reviewSubmission'])->name('homework.submissions.review')->middleware('permission:homework.grade');

            Route::get('homework/lesson-plans',                       [HomeworkController::class, 'lessonPlans'])->name('homework.lesson-plans.index')->middleware('permission:lessons.view');
            Route::post('homework/lesson-plans',                      [HomeworkController::class, 'storeLessonPlan'])->name('homework.lesson-plans.store')->middleware('permission:lessons.create');
            Route::put('homework/lesson-plans/{lessonPlan}/review',   [HomeworkController::class, 'reviewLessonPlan'])->name('homework.lesson-plans.review')->middleware('permission:lessons.approve');
            Route::delete('homework/lesson-plans/{lessonPlan}',       [HomeworkController::class, 'destroyLessonPlan'])->name('homework.lesson-plans.destroy')->middleware('permission:lessons.create');

            Route::get('homework/syllabi',                            [HomeworkController::class, 'syllabi'])->name('homework.syllabi.index')->middleware('permission:syllabus.view');
            Route::post('homework/syllabi',                           [HomeworkController::class, 'storeSyllabus'])->name('homework.syllabi.store')->middleware('permission:syllabus.manage');
            Route::put('homework/syllabi/{syllabus}',                 [HomeworkController::class, 'updateSyllabus'])->name('homework.syllabi.update')->middleware('permission:syllabus.manage');

            Route::get('homework/online-classes',                     [HomeworkController::class, 'onlineClasses'])->name('homework.online-classes.index')->middleware('permission:lessons.view');
            Route::post('homework/online-classes',                    [HomeworkController::class, 'storeOnlineClass'])->name('homework.online-classes.store')->middleware('permission:lessons.create');
            Route::put('homework/online-classes/{onlineClass}/status',[HomeworkController::class, 'updateOnlineClassStatus'])->name('homework.online-classes.status')->middleware('permission:lessons.create');
            Route::delete('homework/online-classes/{onlineClass}',    [HomeworkController::class, 'destroyOnlineClass'])->name('homework.online-classes.destroy')->middleware('permission:lessons.create');
            ...
            // Academic Report Card Previews and PDF Downloads
            Route::get('reports/academic/student/{student}/exam/{exam}/pdf', [ReportController::class, 'exportStudentReportPdf'])->name('reports.academic.student.pdf');
            Route::get('reports/academic/student/{student}/exam/{exam}/preview', [ReportController::class, 'previewStudentReport'])->name('reports.academic.student.preview');
            Route::get('reports/attendance',                    [ReportController::class, 'attendance'])->name('reports.attendance');
            
            Route::get('reports/dashboard', [ReportController::class, 'academic'])->name('reports.dashboard');
    ...
    |--------------------------------------------------------------------------
    */
            Route::middleware(['verified', 'role:student', 'module'])->prefix('school/student')->name('student.')->group(function () {
        ...
        Route::get('results/{exam}/pdf', [StudentPortalController::class, 'downloadReportPdf'])->name('results.pdf');
    ...
    Route::middleware(['verified', 'role:parent', 'module'])->prefix('school/parent')->name('parent.')->group(function () {
        Route::get('dashboard',     [ParentPortalController::class, 'dashboard'])->name('dashboard');
        Route::get('attendance',    [ParentPortalController::class, 'attendance'])->name('attendance');
        
        Route::get('students/{student}/results/{exam}/pdf', [ParentPortalController::class, 'downloadReportPdf'])->name('results.pdf');
    ...
    |--------------------------------------------------------------------------
    */
    Route::middleware(['verified', 'role:super-admin'])
        ->prefix('super-admin')
        ->name('super-admin.')
        ->group(function () {
        ...
        Route::get('system-health', [\App\Http\Controllers\SuperAdmin\SystemHealthController::class, 'index'])->name('system-health.index');
            ...
            // Public website CMS
            Route::get('website/pages',                         [WebsitePageController::class, 'index'])->name('website.pages.index');
            Route::post('website/pages',                        [WebsitePageController::class, 'store'])->name('website.pages.store');
            Route::put('website/pages/{websitePage}',           [WebsitePageController::class, 'update'])->name('website.pages.update');
            Route::patch('website/pages/{websitePage}/publish', [WebsitePageController::class, 'publish'])->name('website.pages.publish');
            Route::delete('website/pages/{websitePage}',        [WebsitePageController::class, 'destroy'])->name('website.pages.destroy');
            Route::post('website/pages/{websitePage}/sections', [WebsitePageController::class, 'saveSection'])->name('website.pages.sections.save');
            Route::delete('website/pages/{websitePage}/sections/{section}', [WebsitePageController::class, 'deleteSection'])->name('website.pages.sections.delete');
            Route::get('website/media',                         [\App\Http\Controllers\SuperAdmin\WebsiteMediaController::class, 'index'])->name('website.media.index');
            Route::post('website/media',                        [\App\Http\Controllers\SuperAdmin\WebsiteMediaController::class, 'store'])->name('website.media.store');
            Route::delete('website/media/{media}',              [\App\Http\Controllers\SuperAdmin\WebsiteMediaController::class, 'destroy'])->name('website.media.destroy');
...
// Public CMS resources must be registered before the generic website catch-all.
Route::get('/blog', [PublicBlogController::class, 'index'])->name('blog.index');
Route::get('/blog/{slug}', [PublicBlogController::class, 'show'])->name('blog.show');
Route::get('/faq', [PublicFaqController::class, 'index'])->name('faq.index');
...
Route::middleware(['auth', 'verified', 'role:super-admin'])->prefix('super-admin')->name('super-admin.')->group(function () {
    Route::resource('blogs', SuperAdminBlogController::class)->except(['create', 'edit', 'show']);
    Route::prefix('faqs')->name('faqs.')->group(function () {
        ...
        Route::patch('/{faq}/toggle-publish', [SuperAdminFaqController::class, 'togglePublish'])->name('toggle-publish');
        Route::patch('/{faq}/toggle-homepage', [SuperAdminFaqController::class, 'toggleHomepage'])->name('toggle-homepage');
        Route::post('/reorder', [SuperAdminFaqController::class, 'reorder'])->name('reorder');
    });
});

// Database-driven public website pages. Keep this after explicit public routes.
Route::get('/{path}', [PublicWebsiteController::class, 'page'])
    ->where('path', '(?!(?:school|super-admin|api|admin|student|parent|onboarding|billing|email|logout|apply|auth)(?:/|$)).*')
    ->middleware('throttle:120,1')
    ->name('public.page');
// --- DYNAMIC SECTIONS & STUDENT ADMISSIONS VAULT ROUTES ---
Route::middleware(['auth', 'verified'])->group(function () {
    // Dynamic Sections API & Actions
    Route::get('/api/classes/{class}/sections', [App\Http\Controllers\SectionController::class, 'byClass'])->name('sections.by-class');
    ...
    // Document Management
    Route::post('/students/{student}/documents', [App\Http\Controllers\StudentController::class, 'uploadDocument'])->name('students.documents.upload');
    Route::get('/students/{student}/documents/{document}/download', [App\Http\Controllers\StudentController::class, 'downloadDocument'])->name('students.documents.download');
    Route::delete('/students/{student}/documents/{document}', [App\Http\Controllers\StudentController::class, 'deleteDocument'])->name('students.documents.delete');

    // Standard Student Resource
    Route::resource('students', App\Http\Controllers\StudentController::class);
});

// Virtual Classroom & Online Lessons
Route::middleware(['auth', 'verified'])->group(function () {
    Route::get('school/classroom/{onlineClass}', [\App\Http\Controllers\VirtualClassroomController::class, 'join'])->name('classroom.join');

    Route::prefix('school/online-classes')->name('school.online-classes.')->group(function () {
        Route::get('/', [\App\Http\Controllers\SchoolAdmin\OnlineClassController::class, 'index'])->name('index');
        Route::post('/', [\App\Http\Controllers\SchoolAdmin\OnlineClassController::class, 'store'])->name('store');
        Route::post('{onlineClass}/start', [\App\Http\Controllers\SchoolAdmin\OnlineClassController::class, 'start'])->name('start');
        Route::post('{onlineClass}/end', [\App\Http\Controllers\SchoolAdmin\OnlineClassController::class, 'end'])->name('end');
        Route::delete('{onlineClass}', [\App\Http\Controllers\SchoolAdmin\OnlineClassController::class, 'destroy'])->name('destroy');
    });
});
