                            ...
                            {staffList.data.map(s => {
                                ...
                                return (
                                    <TableRow key={s.id}>
                                        ...
                                        <TableCell className="text-right text-sm">{struct ? `KSh ${Number(struct.basic_salary).toLocaleString()}` : '—'}</TableCell>
                                        <TableCell className="text-right text-sm text-green-600">{struct ? `KSh ${allowTotal.toLocaleString()}` : '—'}</TableCell>
                                        <TableCell className="text-right text-sm text-red-600">{struct ? `KSh ${dedTotal.toLocaleString()}` : '—'}</TableCell>
                                        <TableCell className="text-right font-bold text-indigo-600">{netSal != null ? `KSh ${netSal.toLocaleString()}` : <Badge className="bg-amber-100 text-amber-700 border-0 text-xs">Not Set</Badge>}</TableCell>
                                        <TableCell>
                ...
                <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="space-y-5 mt-2">
                        <div className="space-y-1.5">
                            <Label>Basic Salary (KSh ) <span className="text-red-500">*</span></Label>
                        ...
                        <div>
                            ...
                            {form.allowances.map((a, i) => (
                                <div key={i} className="flex gap-2 mb-2">
                                    <Input placeholder="Label (e.g. Housing Allowance)" value={a.label} onChange={e => updateItem('allowances', i, 'label', e.target.value)} className="flex-1" />
                        ...
                        <div>
                            ...
                            {form.deductions.map((d, i) => (
                                <div key={i} className="flex gap-2 mb-2">
                                    <Input placeholder="Label (e.g. PAYE / NSSF / SHIF)" value={d.label} onChange={e => updateItem('deductions', i, 'label', e.target.value)} className="flex-1" />
                        ...
                        <div className="rounded-lg bg-slate-50 dark:bg-slate-900 p-3 space-y-1 text-sm">
                            <div className="flex justify-between"><span className="text-slate-500">Gross Salary</span><span className="font-medium">KSh {grossSalary.toLocaleString()}</span></div>
                            <div className="flex justify-between text-red-600"><span>Total Deductions</span><span>- KSh {totalDeductions.toLocaleString()}</span></div>
                            <div className="flex justify-between font-bold text-base border-t border-slate-200 dark:border-slate-700 pt-1 mt-1">
                                <span>Net Salary</span><span className="text-indigo-600">KSh {netSalary.toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                    <DialogFooter>
