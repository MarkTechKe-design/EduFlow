...
export default function FinanceReport({ collected, outstanding, payroll, dailyChart, payments, filters }: Props) {
    ...
    function exportPdf() {
        window.open(`/school/reports/finance/export-pdf?from_date=${fromDate}&to_date=${toDate}`, "_blank", "noopener,noreferrer");
    }
                    ...
                    <CardContent className="p-0">
                        <Table>
                            ...
                            <TableBody>
                                ...
                                {payments.data.map(p => (
                                    <TableRow key={p.id}>
                                        <TableCell>{p.student ? `${p.student.first_name} ${p.student.last_name}` : 'Ã¢â‚¬—'}</TableCell>
                                        <TableCell>{p.student?.admission_no ?? 'Ã¢â‚¬—'}</TableCell>
                                        ...
                                        <TableCell>{p.payment_date ? new Date(p.payment_date).toLocaleDateString() : 'Ã¢â‚¬—'}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
