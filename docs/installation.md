...
This guide walks you through installing EduFlow on a Linux VPS (Ubuntu 22.04 or 24.04 recommended).
...
```sql
CREATE DATABASE eduflow CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'eduflow_user'@'localhost' IDENTIFIED BY 'your_strong_password';
GRANT ALL PRIVILEGES ON eduflow.* TO 'eduflow_user'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```
...
```bash
cd /var/www
sudo git clone https://github.com/your-org/eduflow.git eduflow
sudo chown -R $USER:www-data /var/www/eduflow
```
...
```bash
sudo chown -R www-data:www-data /var/www/eduflow/storage
sudo chown -R www-data:www-data /var/www/eduflow/bootstrap/cache
sudo chmod -R 775 /var/www/eduflow/storage
sudo chmod -R 775 /var/www/eduflow/bootstrap/cache
```
...
```bash
sudo nano /etc/supervisor/conf.d/eduflow-horizon.conf
```

```ini
[program:eduflow-horizon]
process_name=%(program_name)s
command=php /var/www/eduflow/artisan horizon
autostart=true
autorestart=true
user=www-data
redirect_stderr=true
stdout_logfile=/var/www/eduflow/storage/logs/horizon.log
stopwaitsecs=3600
```
...
sudo supervisorctl start eduflow-horizon
```
...
```
* * * * * php /var/www/eduflow/artisan schedule:run >> /dev/null 2>&1
```
...
sudo supervisorctl restart eduflow-horizon
```
...
```bash
tail -f /var/www/eduflow/storage/logs/laravel.log
```
...
```bash
sudo supervisorctl status eduflow-horizon
sudo supervisorctl restart eduflow-horizon
```
