...
class ParentPortalController extends Controller
{
    ...
    public function downloadReportPdf(Request $request, Student $student, Exam $exam)
    {
        $user = $request->user();
        $guardian = \App\Models\Guardian::where('user_id', $user->id)->firstOrFail();

        abort_unless($student->guardian_id === $guardian->id && $student->school_id === $exam->school_id, 403);
