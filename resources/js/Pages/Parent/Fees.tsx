                    ...
                    <div key={child.id}>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                            <Card><CardContent className="p-3 text-center">
                                ...
                                <p className="font-bold text-slate-800 dark:text-white">KSh {child.total_due.toLocaleString()}</p>
                            </CardContent></Card>
                            <Card><CardContent className="p-3 text-center">
                                <p className="text-[11px] text-slate-500 mb-1">Total Paid</p>
                                <p className="font-bold text-green-600">KSh {child.total_paid.toLocaleString()}</p>
                            </CardContent></Card>
                            <Card className={child.balance > 0 ? 'border-red-200' : 'border-green-200'}>
                                <CardContent className="p-3 text-center">
                                    ...
                                    <p className={cn('font-bold', child.balance > 0 ? 'text-red-600' : 'text-green-600')}>
                                        {child.balance > 0 ? `KSh ${child.balance.toLocaleString()}` : 'Clear'}
                                    </p>
                                </CardContent>
                            </Card>
                        </div>
                                        ...
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                            {child.payments.map((p, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td className="px-4 py-2 text-right text-slate-600">KSh {p.due.toLocaleString()}</td>
                                                    <td className="px-4 py-2 text-right text-green-600">KSh {p.paid.toLocaleString()}</td>
